...
hotels <- read_csv("NM2207 W4 hotels.csv")
```
...
# Enter code here
select( 
 hotels, # name of data set
 lead_time  # name of column/variable
 )
```
...
# ?n to see what n() function does

hotels %>%
 group_by(hotel) %>%
 summarise(count = n())

# hotels %>% count(hotel) gvs the same output slide 66
...
#order of piping matters! compare output w before
```
